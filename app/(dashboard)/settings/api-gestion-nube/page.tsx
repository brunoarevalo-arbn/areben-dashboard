...
export default async function ApiGNPage() {
  ...
  return (
    <div className="space-y-6 max-w-xl">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">API Gestión Nube</h1>
        <p className="text-sm text-slate-600 mt-0.5">Configuración de integración con Gestión Nube</p>
      </div>

      <div className="bg-white border border-[#e8e4dc] rounded-xl p-6 space-y-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#f5f0e6] rounded-lg flex items-center justify-center">
            <Plug className="w-5 h-5 text-slate-600" />
          </div>
          <div>
            <p className="font-medium text-slate-900">Gestión Nube</p>
        ...
        {config?.notas && (
          <p className="text-sm text-slate-500 bg-[#f5f0e6] rounded-lg p-3">{config.notas}</p>
        )}
      </div>
    </div>
  )
