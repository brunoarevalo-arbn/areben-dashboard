...
const ACCIONES: QuickAction[] = [
  ...
  },
  {
    ...
    icon: UserPlus,
    color: 'text-primary bg-orange-500/10 border-orange-500/30',
  },
]
...
export function QuickActions() {
  ...
  return (
    <div ref={ref} className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-2">
      {/* Submenú retiros */}
      {open && retirosOpen && (
        <div className="bg-surface border border-border-strong rounded-xl shadow-2xl overflow-hidden min-w-[220px] mb-1 animate-in fade-in slide-in-from-bottom-2 duration-150">
          <div className="px-4 py-2 border-b border-border bg-surface-2/40">
            <p className="text-xs font-semibold text-fg-muted uppercase tracking-wider">Retiro de socio</p>
          </div>
          {SOCIOS_RETIRO.map((socio) => (
            <Link
              ...
              className="flex items-center gap-3 px-4 py-3 hover:bg-surface-2/60 transition-colors border-b border-border/40 last:border-0"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-fg truncate">{socio}</p>
                <p className="text-xs text-fg-soft">Pre-llenar formulario</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      {/* Menú principal */}
      {open && !retirosOpen && (
        <div className="bg-surface border border-border-strong rounded-xl shadow-2xl overflow-hidden min-w-[260px] animate-in fade-in slide-in-from-bottom-2 duration-150">
          <div className="px-4 py-2 border-b border-border bg-surface-2/40">
            <p className="text-xs font-semibold text-fg-muted uppercase tracking-wider">Acceso rápido</p>
          </div>
          ...
          {ACCIONES.map((a) => {
            ...
            return (
              <Link
                ...
                className="flex items-center gap-3 px-4 py-3 hover:bg-surface-2/60 transition-colors border-b border-border/40"
                title={a.description}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-fg-soft truncate">{a.description}</p>
                </div>
              </Link>
            )
          })}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-fg">Retiro de socio</p>
              <p className="text-xs text-fg-soft">Elegir titular →</p>
            </div>
      ...
      <button
        ...
        className={cn(
          ...
          open
            ? 'bg-surface-2 border-[#c8c0b0] rotate-45'
            : 'bg-orange-500 border-orange-500 hover:bg-orange-500 hover:scale-105',
        )}
      >
        {open ? <X className="w-6 h-6 text-fg" /> : <Plus className="w-6 h-6 text-white" />}
      </button>
    </div>
  )
}
