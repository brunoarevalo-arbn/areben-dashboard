...
export function QuickActions() {
  ...
  return (
    <div ref={ref} className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-2">
      {/* Submenú retiros */}
      {open && retirosOpen && (
        <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden min-w-[220px] mb-1 animate-in fade-in slide-in-from-bottom-2 duration-150">
          <div className="px-4 py-2 border-b border-slate-800 bg-slate-800/40">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Retiro de socio</p>
          </div>
          {SOCIOS_RETIRO.map((socio) => (
            <Link
              ...
              className="flex items-center gap-3 px-4 py-3 hover:bg-slate-800/60 transition-colors border-b border-slate-800/40 last:border-0"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-100 truncate">{socio}</p>
      ...
      {/* Menú principal */}
      {open && !retirosOpen && (
        <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden min-w-[260px] animate-in fade-in slide-in-from-bottom-2 duration-150">
          <div className="px-4 py-2 border-b border-slate-800 bg-slate-800/40">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Acceso rápido</p>
          </div>
          ...
          {ACCIONES.map((a) => {
            ...
            return (
              <Link
                ...
                className="flex items-center gap-3 px-4 py-3 hover:bg-slate-800/60 transition-colors border-b border-slate-800/40"
                title={a.description}
              >
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-slate-100">Retiro de socio</p>
      ...
      >
        {open ? <X className="w-6 h-6 text-slate-100" /> : <Plus className="w-6 h-6 text-white" />}
      </button>
    </div>
  )
}
