  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div className="bg-slate-800/40 border border-slate-700/40 rounded-xl p-4 space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="col-span-2 space-y-1.5">
            <label className="block text-xs font-medium text-slate-400">Monto principal</label>
            <input
              type="number"
              step="0.01"
              value={montoEstimado || ''}
        ...
        <label className="flex items-center gap-2 cursor-pointer text-xs">
          ...
          />
          <span className="text-slate-300">Componente en otra moneda (ej: alquiler con parte fija USD + ajuste ARS)</span>
        </label>

        {tieneSecundario && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pl-6 pt-1">
            <div className="col-span-2 space-y-1.5">
              <label className="block text-xs font-medium text-slate-400">Monto secundario</label>
              <input
                type="number"
                step="0.01"
      ...
      <div className="bg-slate-800/40 border border-slate-700/40 rounded-xl p-4 space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <label className="block text-xs font-medium text-slate-400">Tipo de IVA</label>
            <select
              value={porcentajeIva}
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-medium text-slate-400">Día de vencimiento</label>
            <input
              type="number"
              min="1" max="31"
  ...
  return (
    <div className="space-y-4">
      <div className="bg-slate-800/60 rounded-lg px-4 py-3">
        <p className="text-sm text-slate-300 font-medium">{recurrente.concepto}</p>
      ...
      {tieneSecundario && monedaSecundaria && (
        <div className="space-y-2">
          <label className="text-sm font-medium text-slate-300">¿Cómo registrar este mes?</label>
          <div className="grid grid-cols-1 gap-2">
            {([
              ...
              >
                <p className={cn('text-sm font-medium', modo === v ? 'text-indigo-300' : 'text-slate-200')}>{label}</p>
      ...
      <div className="space-y-1.5">
        ...
        <input
          ...
          className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-slate-100 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
          ...
          {tc > 0 && (
            <div className="bg-slate-900/40 rounded-lg px-3 py-2 text-xs space-y-1">
              <div className="flex justify-between">
                <span className="text-slate-400">{formatCurrency(montoP, monedaPrincipal)}</span>
                <span className="text-slate-300 font-mono">+ {formatCurrency(montoS, monedaSecundaria)}</span>
              </div>
              <div className="flex justify-between border-t border-slate-700 pt-1">
                <span className="text-slate-300 font-medium">Total en {monedaPrincipal}:</span>
      ...
      <div className="bg-slate-800/40 border border-slate-700/40 rounded-lg px-3 py-2 text-xs text-slate-400">
        <span className="text-slate-300 font-medium">Se va a crear: </span>
  ...
  return (
    <div className="space-y-3">
      <div className="bg-slate-800/60 rounded-lg p-4 space-y-2 text-sm">
        <div className="flex justify-between"><span className="text-slate-400">Concepto</span><span className="text-slate-100">{recurrente.concepto}</span></div>
        <div className="flex justify-between"><span className="text-slate-400">Categoría</span><span>{recurrente.categoria}</span></div>
        <div className="flex justify-between"><span className="text-slate-400">Monto estimado</span><span className="font-mono">{formatCurrency(recurrente.monto_estimado)}</span></div>
        <div className="flex justify-between"><span className="text-slate-400">IVA</span><span>{recurrente.iva_incluido ? `Incluido (${recurrente.porcentaje_iva}%)` : 'No incluido'}</span></div>
        <div className="flex justify-between"><span className="text-slate-400">Medio pago</span><span>{recurrente.medio_pago}</span></div>
        <div className="flex justify-between"><span className="text-slate-400">Tipo mes</span><span>{recurrente.tipo_mes}</span></div>
        {recurrente.dia_vencimiento && <div className="flex justify-between"><span className="text-slate-400">Día venc.</span><span>{recurrente.dia_vencimiento}</span></div>}
      </div>

      {recurrente.prorrateo && (
        <div className="bg-slate-800/60 rounded-lg p-4">
          <p className="text-xs font-medium text-slate-400 mb-2">Prorrateo</p>
      ...
      {recurrente.detalles && (
        <div className="bg-slate-800/60 rounded-lg p-4">
          ...
          <pre className="text-xs text-slate-300 font-mono whitespace-pre-wrap">{JSON.stringify(recurrente.detalles, null, 2)}</pre>
        </div>
      )}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Gastos recurrentes</h1>
          <p className="text-sm text-slate-400 mt-0.5">Plantillas de gastos mensuales — confirmar para {formatMonth(mes)}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-1">
          <p className="text-xs text-slate-400">Total estimado mes</p>
        ...
        <div className="bg-slate-900 border border-green-500/20 rounded-xl p-4">
          <p className="text-xs text-slate-400 mb-1">Confirmados</p>
        ...
        <div className="bg-slate-900 border border-amber-500/20 rounded-xl p-4">
          <p className="text-xs text-slate-400 mb-1">Pendientes de confirmar</p>
      ...
      {seleccionados.size > 0 && (
        <div className="bg-indigo-500/10 border border-indigo-500/40 rounded-xl px-4 py-3 flex items-center justify-between sticky top-0 z-10 backdrop-blur">
          <div className="flex items-center gap-2 text-sm">
            <ListChecks className="w-4 h-4 text-indigo-400" />
            <span className="text-indigo-300 font-medium">{seleccionados.size} recurrente(s) seleccionado(s)</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-800">
              ...
              <th className="text-right px-4 py-3 text-xs font-medium text-slate-400 uppercase">Estimado</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-slate-400 uppercase">Medio</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-slate-400 uppercase">Compartido</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-slate-400 uppercase">Estado mes</th>
                  ...
                  <tr key={r.id} className={cn(
                    ...
                    <td className="px-3 py-2.5">
                      <input
                        ...
                        className="w-4 h-4 rounded border-slate-600 bg-slate-700 text-indigo-500 focus:ring-indigo-500 disabled:opacity-30"
                      />
                    </td>
                    <td className="px-4 py-2.5">
                      <p className="font-medium text-slate-100">{r.concepto}</p>
                      <p className="text-xs text-slate-500">
                        {r.iva_incluido && <span>IVA {r.porcentaje_iva}% incluido · </span>}
                        {r.tipo_mes === 'VENCIDO' ? 'mes vencido' : 'mes corriente'}
                      </p>
                    </td>
                    <td className="px-4 py-2.5 text-slate-300">{r.categoria}</td>
