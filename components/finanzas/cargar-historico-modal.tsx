        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              className={cn(
                'flex items-center justify-center gap-2 px-3 py-2 rounded-lg border text-sm font-medium transition-colors',
                modo === 'EXCEL'
                  ? 'bg-green-600/20 border-green-500/50 text-success'
                  : 'bg-surface-2 border-border-strong text-fg-muted hover:text-fg-muted',
              )}
            >
          ...
          {modo === 'EXCEL' && (
            <div className="text-center py-6">
              <Upload className="w-10 h-10 mx-auto mb-2 text-fg-soft" />
              <p className="text-sm text-fg-muted mb-4">Importá múltiples filas desde una planilla Excel</p>
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-fg-muted mb-1.5">Primer mes a vencer</label>
      ...
      <p className="text-xs text-fg-soft bg-surface-2/40 border border-border-strong/40 rounded-lg px-3 py-2">
        ...
        desde <span className="text-fg-muted">{primerMesVencimiento}</span> en adelante.
        {cuotasTotalOriginal > cuotasRestantes && (
          <span className="block mt-1 text-fg-soft">Las {cuotasTotalOriginal - cuotasRestantes} cuota(s) anteriores ({cuotaActual > 1 ? `1-${cuotaActual - 1}` : '—'}) se asumen pagadas.</span>
        )}
      </p>
      {error && <p className="text-sm text-red-700 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">{error}</p>}
      <div className="flex justify-end gap-3">
  ...
  return (
    <>
      ...
      <label className="flex items-center gap-2 text-sm text-fg-muted cursor-pointer">
        <input type="checkbox" checked={ivaIncluido} onChange={(e) => setIvaIncluido(e.target.checked)} className="w-4 h-4 rounded border-[#c8c0b0] bg-surface-2" />
        Monto incluye IVA
      </label>
      {ivaIncluido && (
