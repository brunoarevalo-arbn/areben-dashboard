...
export function SimuladorMovimiento({ instrumento, tramos, periodoMesActual, inversorNombre }: Props) {
  ...
  return (
    <div className="bg-slate-900 border border-indigo-500/20 rounded-xl overflow-x-auto">
      <button
        type="button"
        onClick={() => setOpen(!open)}
        className="w-full px-4 py-3 border-b border-slate-800 flex items-center justify-between hover:bg-slate-800/40 transition-colors"
      >
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="block text-xs font-medium text-slate-400">Fecha del movimiento</label>
              <input
                type="date"
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-slate-100 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm disabled:opacity-60"
                placeholder="0.00"
              />
            </div>
          </div>
          ...
          {/* Resultado */}
          <div ref={ref} className="bg-slate-800/40 border border-slate-700/40 rounded-xl p-4 space-y-3 text-sm">
            <div className="border-b border-slate-700/60 pb-2 mb-1">
              <p className="text-xs text-slate-400">SIMULACIÓN — {tipoDescripcion}  ·  {formatDate(fecha)}</p>
              <p className="text-[10px] text-slate-500 mt-0.5">{inversorNombre} · {instrumento.codigo ?? instrumento.id.substring(0, 8)}</p>
            </div>

            <div className="space-y-1">
              <div className="row flex justify-between">
                <span className="text-slate-400">Capital inicio del mes</span>
                <span className="font-mono text-slate-100">{formatMoneda(saldoInicioMes, moneda)}</span>
              </div>
              <div className="row flex justify-between">
                <span className="text-slate-400">Tasa vigente</span>
                <span className="font-mono text-slate-200">{(tasaVigente * 100).toFixed(2)}%</span>
              </div>
            </div>
            ...
            {tieneError ? (
              ...
              <>
                ...
                {result.tramos.map((seg, i) => (
                  <div key={i} className="bg-slate-900/40 rounded-lg p-3 border-l-2 border-indigo-500/40">
                    ...
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-400">Base de cálculo</span>
                      <span className="font-mono text-slate-200">{formatMoneda(
                  ...
                  {result.esRetiroTotal ? (
                    <>
                      ...
                      <div className="row flex justify-between">
                        ...
                        <span className="font-mono text-slate-100">{formatMoneda(result.capitalAlMomento ?? 0, moneda)}</span>
                      </div>
                      <div className="row flex justify-between">
                        <span className="text-slate-300">Intereses devengados</span>
                        <span className="font-mono text-amber-400">{formatMoneda(result.totalIntereses, moneda)}</span>
                      </div>
                      <div className="total bg-slate-950 border border-indigo-500/40 rounded-lg px-3 py-2 flex justify-between text-base font-bold mt-2">
                        <span className="text-slate-100">TOTAL A PAGAR</span>
                        <span className="font-mono text-indigo-400">{formatMoneda(result.totalAPagar ?? 0, moneda)}</span>
                      </div>
                    ...
                    <>
                      ...
                      <div className="row flex justify-between">
                        <span className="text-slate-300">Total intereses devengados</span>
                        <span className="font-mono text-amber-400">{formatMoneda(result.totalIntereses, moneda)}</span>
                      </div>
                      <div className="row flex justify-between">
                        <span className="text-slate-300">{tipo === 'INGRESO' ? 'Ingreso' : 'Retiro'}</span>
                        <span className={cn(
                          'font-mono',
                      ...
                      <div className="row flex justify-between text-base font-semibold pt-1">
                        <span className="text-slate-100">Saldo al cierre del mes</span>
                        <span className="font-mono text-slate-100 flex items-center gap-1.5">
                          {formatMoneda(result.saldoCierre, moneda)}
                          <CheckCircle2 className="w-4 h-4 text-green-400" />
                        </span>
                      </div>
                      <div className="row flex justify-between text-xs pt-2 mt-2 border-t border-slate-700/40">
                        <span className="text-slate-400">Gasto financiero del mes</span>
                        <span className="font-mono text-amber-400">{formatMoneda(result.totalIntereses, moneda)}</span>
                      </div>
                    </>
                  )}
            ...
            <p className="footer text-[10px] text-slate-500 italic pt-2 border-t border-slate-700/40">
