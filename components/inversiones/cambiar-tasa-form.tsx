...
export function CambiarTasaForm({
  ...
  return (
    <form action={formAction} className="space-y-4">
      <div className="bg-surface-2/60 rounded-lg px-4 py-3 text-sm">
        <p className="text-fg-muted font-medium mb-1">Cambio de tasa</p>
        <p className="text-xs text-fg-soft">
          Instrumento: {instrumento.codigo ?? instrumento.id.substring(0, 8)} ·
          Tasa vigente: <span className="font-mono text-fg-muted">{(tasaActual * 100).toFixed(4)}%</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          <label className="block text-xs font-medium text-fg-muted">Fecha desde</label>
          <input
            type="date"
            value={fechaDesde}
          ...
          />
          <p className="text-[10px] text-fg-soft">Aplica desde esta fecha en adelante</p>
        </div>
      </div>
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="font-mono text-sm text-amber-700 font-semibold">
              {formatMoneda(interesEstimado, instrumento.moneda)} <span className="text-xs text-fg-soft">/ mes</span>
            </p>
            {variacion !== 0 && (
      ...
      <div className="bg-surface-2/40 border border-border-strong/40 rounded-lg p-3 text-xs text-fg-muted flex items-start gap-2">
        <AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5 text-amber-700" />
        <div>
          Al guardar, se recalcularán los <strong className="text-fg-muted">períodos abiertos</strong> aplicando esta tasa desde la fecha indicada.
          Los períodos ya cerrados <strong className="text-fg-muted">no se modifican</strong>.
        </div>
      </div>
