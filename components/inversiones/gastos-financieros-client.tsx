    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-fg-muted mt-0.5">Costo de las inversiones de terceros mes a mes</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-surface border border-green-500/20 rounded-xl p-4">
          <p className="text-xs text-fg-muted mb-1">Total invertido USD</p>
        ...
        <div className="bg-surface border border-orange-500/20 rounded-xl p-4">
          <p className="text-xs text-fg-muted mb-1">Total invertido ARS</p>
          <p className="text-xl font-bold text-primary">{formatMoneda(totalInvertidoArs, 'ARS')}</p>
        </div>
      ...
      <div className="bg-surface border border-border rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4 text-sm font-medium text-fg-muted">
      ...
      <div className="bg-surface border border-border rounded-xl overflow-x-auto">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          <h2 className="text-sm font-semibold text-fg">Detalle por inversor / instrumento (todos los meses)</h2>
          <span className="text-xs text-fg-soft">{periodos.length} registros</span>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-surface z-10">
              <tr className="border-b border-border">
                ...
                <th className="text-left px-4 py-2 text-xs font-medium text-fg-muted uppercase">Inversor</th>
                <th className="text-left px-4 py-2 text-xs font-medium text-fg-muted uppercase">Instr.</th>
                <th className="text-left px-4 py-2 text-xs font-medium text-fg-muted uppercase">Mon.</th>
                <th className="text-left px-4 py-2 text-xs font-medium text-fg-muted uppercase">Cap.</th>
            ...
            <tbody>
              {periodos.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-12 text-center text-fg-soft">Sin datos</td></tr>
              ) : (
                periodos.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="border-b border-border/60 hover:bg-surface-2/30">
                      <td className="px-4 py-2 text-xs text-fg-muted">{formatMonth(p.mes)}</td>
                      <td className="px-4 py-2 text-fg-muted">{i.inversor?.nombre ?? '—'}</td>
                      <td className="px-4 py-2 text-xs font-mono text-fg-muted">{i.codigo ?? i.id.substring(0, 8)}</td>
                      <td className="px-4 py-2"><Badge variant={i.moneda === 'USD' ? 'success' : 'info'}>{i.moneda}</Badge></td>
                      <td className="px-4 py-2">
                        <span className={i.capitalizable ? 'text-purple-700' : 'text-fg-muted'}>
                          {i.capitalizable ? <Lock className="w-3 h-3 inline" /> : <Unlock className="w-3 h-3 inline" />}
                        </span>
                      </td>
                      <td className="px-4 py-2 text-right font-mono text-fg-muted text-xs">{formatMoneda(Number(p.saldo_inicio), i.moneda)}</td>
                      <td className="px-4 py-2 text-right font-mono text-amber-700 text-xs">{formatMoneda(Number(p.interes_devengado), i.moneda)}</td>
                      <td className="px-4 py-2 text-right font-mono text-fg font-medium text-xs">{formatMoneda(Number(p.saldo_cierre), i.moneda)}</td>
                    </tr>
                  )
                })
              )}
