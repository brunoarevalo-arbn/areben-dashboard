...
function MovimientoEditor({ p, onSaved }: { p: PeriodoConRel; onSaved: () => void }) {
  ...
  if (p.cerrado) {
    return <span className="font-mono text-slate-400">{Number(p.movimiento) !== 0 ? formatMoneda(Number(p.movimiento), moneda) : '—'}</span>
  }
  ...
  return (
    <div className="flex items-center gap-1 justify-end">
      <input
        ...
        autoFocus
        className="w-28 px-2 py-1 bg-slate-700 border border-slate-600 rounded text-slate-100 font-mono text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500"
      />
      <button
        type="button"
        disabled={isPending}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-x-auto">
        <div className="px-4 py-3 border-b border-slate-800">
          <h2 className="text-sm font-semibold text-slate-100">Detalle del mes — {formatMonth(mes)}</h2>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-800">
              <th className="text-left px-4 py-2 text-xs font-medium text-slate-400 uppercase">Inversor / Cód.</th>
                ...
                return (
                  <tr key={p.id} className={cn(
                    ...
                    <td className="px-4 py-2">
                      <p className="text-slate-100 font-medium">{i.inversor?.nombre ?? '—'}</p>
                    ...
                    <td className="px-4 py-2 text-right font-mono text-slate-300">{formatMoneda(Number(p.saldo_inicio), i.moneda)}</td>
                    ...
                    <td className="px-4 py-2 text-right font-mono text-slate-100 font-bold">{formatMoneda(Number(p.saldo_cierre), i.moneda)}</td>
                    <td className="px-4 py-2">
                      {p.cerrado
          ...
          {periodos.length > 0 && (
            <tfoot>
              <tr className="border-t border-slate-700 bg-slate-800/50">
                <td colSpan={4} className="px-4 py-3 text-sm font-semibold text-slate-300">TOTAL INTERESES (gasto financiero)</td>
