...
const COLORES: Record<string, string> = {
  BDI: 'text-purple-700 border-purple-500/30 bg-purple-500/5',
  ZATTIA: 'text-pink-700 border-pink-500/30 bg-pink-500/5',
  STUNNED: 'text-amber-700 border-amber-500/30 bg-amber-500/5',
}
...
export function ProrrateoSettingsClient({ configs }: { configs: ConfiguracionProrrateo[] }) {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h1 className="text-2xl font-bold text-fg">Configuración de Prorrateo</h1>
      ...
      <div className="bg-surface border border-border rounded-xl p-6 space-y-4">
        ...
        <div className="space-y-3">
          {configs.map((c) => (
            <div key={c.id} className={cn(
              ...
              COLORES[c.marca] ?? 'text-fg-muted border-border-strong bg-surface-2/40'
            )}>
        ...
        <div className={cn(
          ...
          valido ? 'border-green-500/30 bg-green-500/5 text-green-700' : 'border-red-500/30 bg-red-500/5 text-red-700'
        )}>
        ...
        <div className="flex items-center justify-between gap-3 pt-2">
          <button
            ...
            className="text-xs px-3 py-1.5 rounded-lg bg-surface-2 hover:bg-slate-600 text-fg-muted"
        ...
        {success && (
          <div className="flex items-center gap-2 text-sm text-green-700 bg-green-500/10 border border-green-500/20 rounded-lg px-3 py-2">
            <CheckCircle2 className="w-4 h-4" />
            Configuración actualizada
          </div>
        )}
        {error && (
          <div className="flex items-center gap-2 text-sm text-red-700 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
      ...
      <div className="bg-surface/40 border border-border rounded-xl p-4 text-xs text-fg-muted">
        <p className="font-medium text-fg-muted mb-2">¿Cómo funciona?</p>
        <ul className="space-y-1 list-disc list-inside text-fg-soft">
