        ...
        return (
          <div key={tipo}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {lista.length === 0 ? (
                <div className="col-span-3 bg-slate-900 border border-slate-800 rounded-xl p-6 text-center text-slate-500 text-sm">
                ...
                lista.map((p) => (
                  <div key={p.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                    ...
                    <div className="space-y-1.5 text-sm mb-4">
                      {p.pais !== 'Argentina' && <p className="text-slate-400">🌍 {p.pais}</p>}
                      {p.contacto && <p className="text-slate-400">{p.contacto}</p>}
                      {p.condiciones_pago && <p className="text-slate-400 text-xs">{p.condiciones_pago}</p>}
                    </div>
                    <div className="flex items-center gap-2 mb-3">
                      {p.email && <a href={`mailto:${p.email}`} className="p-1.5 rounded hover:bg-slate-800 text-slate-500 hover:text-slate-300 transition-colors"><Mail className="w-3.5 h-3.5" /></a>}
                      {p.telefono && <a href={`tel:${p.telefono}`} className="p-1.5 rounded hover:bg-slate-800 text-slate-500 hover:text-slate-300 transition-colors"><Phone className="w-3.5 h-3.5" /></a>}
                    </div>
